...
select  subj_code ,  subj_name ,   disp_name , balanorient , bank_code  ,bank_name,  
...
select ff.code  subj_code , ee.name subj_name , ee.dispname  disp_name , gg.balanorient, 
       ...
       decode(cc.periodv,'00',
                    ...
                    0) bal_beg , 
      CC.ASSID  , hh.assid , hh.f20  ,cc.yearv , cc.periodv  , nvl(ii.code ,'~')  bank_code  , 
      nvl(ii.name ,'~')  bank_name
...
left join  bd_bankdoc   ii 
       on
       (
            hh.f20 = ii.pk_bankdoc
            and ii.dr = 0 
       )       
) group by subj_code ,  subj_name ,   disp_name , balanorient , bank_code  ,bank_name
