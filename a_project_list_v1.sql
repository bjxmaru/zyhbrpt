...
select  subj_code ,  subj_name ,   disp_name , balanorient , project_code ,project_name , pk_project ,
sum(bal_beg) bal_beg,  sum(localdebitamount) localdebitamount , 
sum(localcreditamount) localcreditamount, sum(bal) bal 
from 
(
select ff.code  subj_code , ee.name subj_name , ee.dispname  disp_name , gg.balanorient, 
       nvl(ii.project_code , '~') project_code ,
       nvl(ii.project_name ,'~' ) project_name , 
       nvl(ii.pk_project , '~')   pk_project , 
...
inner join gl_detail cc  
      on (
           ...
           and cc.periodv <=  substr( parameter('end_date_param') , 6, 2)   || 'Z'
           and cc.periodv >= '00'
           and cc.yearv =  substr( parameter('end_date_param') , 1, 4) 
...
left join  BD_PROJECT   ii 
       on
       (
            hh.f10 = ii.pk_project
            and ii.dr = 0 
       )       
     
) group by subj_code ,  subj_name ,   disp_name , balanorient ,project_code ,project_name , pk_project
