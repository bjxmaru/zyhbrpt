...
inner join  bd_account ff
       on (
            ...
            and ff.dr = 0 
            and ff.code not like decode(parameter('item_param' ) , '供热' , '600104','a') || '%'
            and ff.code not like decode(parameter('item_param' ) , '其他业务' , '605102','a') || '%'
            and ff.code like macro('m_item') || '%'
       )
